"""Enhanced planning module with memory integration.

Extends the original planning module to leverage:
- Long-term memory for similar paper retrieval
- Short-term memory for session context
- Node memory for agent-level context
"""
...
def enhance_prompt_with_memory(base_prompt: str, 
                               similar_papers: list,
                               code_patterns: list) -> str:
    """Enhance prompt with retrieved context from memory.
    
    Args:
        base_prompt: Original prompt
        similar_papers: Retrieved similar papers from long-term memory
        code_patterns: Retrieved code patterns from long-term memory
    ...
    if similar_papers:
        ...
        enhanced += "The following similar papers and their implementations may provide useful insights:\n\n"
        
        for i, (entry_id, entry, score) in enumerate(similar_papers[:3], 1):
            paper_name = entry.metadata.get('paper_name', 'Unknown')
            enhanced += f"{i}. **{paper_name}** (similarity: {score:.2f})\n"
            if 'summary' in entry.metadata:
                enhanced += f"   Summary: {entry.metadata['summary']}\n"
            enhanced += "\n"
    
    if code_patterns:
        enhanced += "\n\n## Relevant Code Patterns (for reference)\n"
        enhanced += "The following code patterns may be helpful:\n\n"
        
        for i, (entry_id, entry, score) in enumerate(code_patterns[:3], 1):
            pattern_name = entry.metadata.get('pattern_name', 'Pattern')
            enhanced += f"{i}. **{pattern_name}** (relevance: {score:.2f})\n"
            if isinstance(entry.content, dict) and 'description' in entry.content:
                enhanced += f"   Description: {entry.content['description']}\n"
            enhanced += "\n"
    
    return enhanced


def run_planning_with_memory(args):
    """Run planning stage with memory integration."""
    ...
    memory_manager = MemoryManager(storage_dir=args.memory_dir)
    short_term_memory = memory_manager.start_session(paper_name)
    
    # Initialize embedding generator
    embedding_generator = EmbeddingGenerator(
        backend="openai",
        model="text-embedding-3-small",
        cache_dir=os.path.join(args.memory_dir, "embeddings")
    )
    ...
    # Store paper in short-term memory
    short_term_memory.set_paper(paper_content)
    
    # Generate paper embedding
    print("[MEMORY] Generating paper embedding...")
    paper_embedding = embed_paper(paper_content, embedding_generator)
    
    # Retrieve similar papers and code patterns from long-term memory
    print("[MEMORY] Retrieving similar papers and code patterns...")
    retrieved_context = memory_manager.retrieve_similar_context(
        query_embedding=paper_embedding,
        keywords=[paper_name, 'transformer', 'neural', 'model'],  # Can be extracted from paper
        top_k=3
    )
    
    # Get node memory for planning agent
    planning_agent = short_term_memory.get_node_memory("planning")
    
    # Original planning messages
    plan_msg = [
...
The response should give us a strong roadmap, making it easier to write the code later."""}]
    ...
    # Enhance first message with memory context
    plan_msg[1]['content'] = enhance_prompt_with_memory(
        plan_msg[1]['content'],
    ...
    # Log to node memory
    planning_agent.add_entry(
        content={'similar_papers': len(retrieved_context['similar_papers']),
                'code_patterns': len(retrieved_context['code_patterns'])},
        entry_type='memory_retrieval'
    )
...
"""
    ...
    for idx, instruction_msg in enumerate([plan_msg, file_list_msg, task_list_msg, config_msg]):
        ...
        if idx == 0:
            short_term_memory.add_planning('overview', message.content)
        elif idx == 1:
            short_term_memory.add_planning('architecture', message.content)
        elif idx == 2:
            short_term_memory.add_planning('task_list', message.content)
        elif idx == 3:
            short_term_memory.add_artifact('config_yaml', message.content)
        
        # Log to node memory
        planning_agent.add_entry(
            content={'stage': current_stage, 'response_length': len(message.content)},
            entry_type='stage_complete'
        )
    ...
    short_term_memory.save(output_dir)
    
    # Save to long-term memory (add paper for future retrieval)
    print("[MEMORY] Saving paper to long-term memory...")
    memory_manager.long_term_memory.add_paper(
        paper_name=paper_name,
        paper_content=paper_content,
        metadata={
            'paper_name': paper_name,
            'format': paper_format,
            'timestamp': short_term_memory.session_start
        },
        embedding=paper_embedding
    )
    
    # End session
    memory_manager.end_session(output_dir, save_to_long_term=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--memory_dir', type=str, default="./memory_store",
                       help="Directory for long-term memory storage")
    
    args = parser.parse_args()
    
    run_planning_with_memory(args)
